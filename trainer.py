...
class Aligner(object):
    ...
    #             # aligning forward!
    #             outputs = self.model(
    #                 input_ids=inputs["base_input_ids"], labels=inputs["labels"]
    #             )
    #             eval_labels += [inputs["labels"]]
    #             eval_preds += [outputs.logits]
    #     eval_metrics = self.compute_metrics_fn(eval_preds, eval_labels)
    #     logger.info(
    #         f"[WARNING: THIS NEEDS TO BE GOOD!] prealign task accuracy: {eval_metrics['accuracy']}"
    #     )

    #     if self.is_master and not self.is_wandb:
    #         log_prealign = open(
    #             os.path.join(output_dir, "prealign_log.txt"), "w", buffering=1
    #         )
    #         print(f"prealign_accuracy,{eval_metrics['accuracy']}", file=log_prealign)
    #         log_prealign.close()
    #     elif self.is_wandb:
    #         wandb.log({"eval/prealign_accuracy": eval_metrics["accuracy"]}, step=0)
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                source_hidden_states = self.model(
                    ...
                    last_token_position=inputs["source_input_last_pos"],
                    output_rotated_hidden_states_only=True,
                ).rotated_hidden_states

                outputs = self.model(
                    input_ids=inputs["base_input_ids"],
                    last_token_position=inputs["base_input_last_pos"],
                ...
                if self.is_master and total_step % log_step == 0:
                    ...
                    if total_step != 0 and total_step % valid_steps == 0:
                        ...
                        if self.is_wandb:
                            ...
                            log_eval = open(
                                os.path.join(output_dir, "eval_log.txt"),
                                "a",
                                buffering=1,
                            )
                            print(
                                "{},{}".format(total_step, eval_metrics["accuracy"]),
                                file=log_eval,
                            )
                            log_eval.close()
