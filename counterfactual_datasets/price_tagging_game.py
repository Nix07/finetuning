...
def pricing_tag_game_config_sampler(amount, lower_bound, bound_width):
    """This function returns a single sample consisting of lower bound, upper bound, and amount."""
    if bound_width == None:
...
def sample_with_region(region, lower_bound_sample, upper_bound_sample):
    """This function returns a sample amount based on the region, i.e. below lower bound, between lower bound and upper bound, and above upper bound."""
    if region == 1:
...
def lower_bound_alignment_example_sampler(
    ...
    """This function samples a lower bound alignment example consisting of a base example and a source example such that their amounts are according to the final label. If label is Yes, the base amount will be less than source and vice versa."""
    (
    ...
    if ctf_label_str == "Yes":
        ...
        base_source_regions = [[1, 1], [2, 1], [2, 3], [3, 1], [3, 2], [3, 3]]
...
def upper_bound_alignment_example_sampler(
    ...
    if ctf_label_str == "Yes":
        ...
        ctf_label = tokenizer.convert_tokens_to_ids("No")
        base_source_regions = [[1, 1], [1, 2], [1, 3], [2, 1], [2, 3], [3, 3]]
    base_source_region = random.choice(base_source_regions)
    base_region = base_source_region[0]
    source_region = base_source_region[1]
...
def bracket_alignment_sampler(
    ...
    for _ in range(max_n_training_examples):
        ...
        ctf_label_str = None
        if (
            base_amount_sample <= source_upper_bound_sample
            and base_amount_sample >= source_lower_bound_sample
        ):
            ctf_label = tokenizer.convert_tokens_to_ids("Yes")
            ctf_label_str = "Yes"
        else:
...
def prepare_dataloader(args, tokenizer):
    ...
    prealign_dataloader = DataLoader(prealign_dataset, batch_size=prealign_batch_size)
    ...
    raw_eval = (
        raw_data[0][
            args.n_training_examples : args.n_training_examples + args.n_eval_examples
        ],
        raw_data[1][
            args.n_training_examples : args.n_training_examples + args.n_eval_examples
        ],
...
def sample_with_region_with_triples(region, triples):
...
def lower_bound_alignment_example_sampler_with_triples(
    ...
    base_lower_bound_sample, base_upper_bound_sample, base_amount_sample = base_triples
    (
        source_lower_bound_sample,
        source_upper_bound_sample,
        source_amount_sample,
...
def bound_alignment_sampler_with_triples(
    tokenizer, max_n_training_examples, bound_functors, triples
):
    all_base_input_ids = []
    all_source_input_ids = []
    all_ctf_output_ids = []  # this one does not have input ids, etc..
    ...
    for _ in range(max_n_training_examples):
        ...
        (
            ...
            ctf_label,
            ctf_label_str,
        ) = bound_functor(tokenizer, triples)
