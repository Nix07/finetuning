...
def bar(tensor, renderer=None, **kwargs):
    ...
    if "margin" in kwargs_post and isinstance(kwargs_post["margin"], int):
        kwargs_post["margin"] = dict.fromkeys(list("tblr"), kwargs_post["margin"])
    px.bar(y=utils.to_numpy(tensor), **kwargs_pre).update_layout(**kwargs_post).show(
        renderer
    )
...
def hist(tensor, renderer=None, **kwargs):
    ...
    if "margin" in kwargs_post and isinstance(kwargs_post["margin"], int):
        kwargs_post["margin"] = dict.fromkeys(list("tblr"), kwargs_post["margin"])
    px.histogram(x=utils.to_numpy(tensor), **kwargs_pre).update_layout(
        **kwargs_post
    ).show(renderer)
...
def plot_logit_attribution(
    model: HookedTransformer, logit_attr: t.Tensor, tokens: t.Tensor, title: str = ""
):
...
def plot_neurons(
    ...
    for name, mask in failure_types_dict.items():
        failure_types = np.where(
            utils.to_numpy(mask[utils.to_numpy(data.starts_open)]), name, failure_types
        )
    ...
    df = pd.DataFrame(
        {
            "Output in 2.0 direction": utils.to_numpy(
